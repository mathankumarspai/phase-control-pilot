...
export const AmpsSettings = () => {
    ...
    ctKey: keyof AmpsSettingsData;
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <Button variant={ctSettings.phase === "3 Phase" ? "default" : "outline"} size="sm" onClick={() => updateCTSetting(ctKey, "phase", "3 Phase")} className={`px-4 py-2 rounded-xl font-medium transition-all ${ctSettings.phase === "3 Phase" ? "bg-yellow-500 text-white shadow-md" : "bg-white/30 text-black border-white/30 hover:bg-yellow-500/20"}`}>
            3 Phase
          </Button>
          <Button variant={ctSettings.phase === "2 Phase" ? "default" : "outline"} size="sm" onClick={() => updateCTSetting(ctKey, "phase", "2 Phase")} className={`px-4 py-2 rounded-xl font-medium transition-all ${ctSettings.phase === "2 Phase" ? "bg-yellow-500 text-white shadow-md" : "bg-white/30 text-black border-white/30 hover:bg-yellow-500/20"}`}>
            2 Phase
          </Button>
        </div>
      </div>
      ...
      <div className="px-2">
        <Slider value={[ctSettings.value]} onValueChange={values => updateValue(ctKey, values[0])} max={50} step={1} className="w-full" />
  ...
  return <Card className="p-6 bg-gradient-to-br from-green-400 to-green-500 shadow-panel rounded-3xl bg-lime-600">
